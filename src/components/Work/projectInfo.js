// import image from "../../assets/danielle-macinnes-IuLgi9PWETU-unsplash.jpg";

const projectInfo = [
  {
    title: "Junior JavaScript Developer Job Board",
    text:
      "Currently learning Recoil for state management and Grommet.io for styling. Shipping code bi-weekly following agile methodologies and pair programming on a remote team of 6 across 3 time zones using Zoom, Trello, and Slack. Soon to start building unit and integraon tests with React Testing Library and Jest.",
    links: [
      // { display: "Just because", href: "/", className: 'deployment' },
      // {pipe: 'pipe'},
      { display: "Deployment Link", href: "/", className: "deployment" },
      { pipe: "pipe" },
      {
        display: "Repo Link",
        href: "https://github.com/JS-Jr-Dev-Job-Board/react",
        className: "repo",
      },
      { pipe: "pipe" },
      { display: "Demo Video", href: "/", className: "demo" },
    ],
    image: "",
  },
  // {
  //   title: "Driftwood",
  //   text: "A node and handlebars app",
  //   links: [
  //     {
  //       display: "Repo Link",
  //       href: "https://reisman-weather-application.herokuapp.com/",
  //       className: "repo",
  //     },
  //     { pipe: "pipe" },
  //     {
  //       display: "Deployment Link",
  //       href: "https://github.com/BrianReisman/node-weather-app",
  //       className: "deployment",
  //     },
  //     // { pipe: "pipe" },
  //     // { display: "Demo Video", href: "/", className: "demo" },
  //   ],
  //   image: "",
  // },
];

export default projectInfo;
